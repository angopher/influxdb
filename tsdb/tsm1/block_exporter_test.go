...
func TestSQLBlockExporter_Export(t *testing.T) {
	...
	want := fmt.Sprintf(`
BEGIN TRANSACTION;
INSERT INTO blocks (filename, org_id, bucket_id, key, type, min_time, max_time, checksum, count) VALUES
('%s', 0, 0, 'cpu', 'integer', 0, 0, 3294968665, 1),
('%s', 0, 0, 'mem', 'integer', 0, 0, 755408492, 1);
COMMIT;
`[1:], f.Name(), f.Name())
